...
import { Loader2, Package, RefreshCw, Sparkles, AlertCircle, CheckCircle2 } from 'lucide-react';
import { ProductSchema, type ProductInput } from '@/lib/validations';
import { useCategories, useBrands, useUnits, useCreateProduct } from '@/hooks/useProducts';
...
export function ProductFormModal({ open, onOpenChange }: ProductFormModalProps) {
  ...
  // Validate SKU uniqueness with debounce
  useEffect(() => {
    if (!watchedSKU || watchedSKU.trim().length < 3) {
      setSkuError(null);
      return;
    }
    ...
    const timer = setTimeout(async () => {
      ...
      try {
        const exists = await checkSKUExists(watchedSKU);
        if (exists) {
  ...
  // Auto-generate SKU when name and category are filled
  const handleGenerateSKU = useCallback(async () => {
    ...
    try {
      ...
      if (exists) {
        // Generate a new one with timestamp suffix
  ...
  // Auto-generate SKU when both name and category are filled (only if SKU is empty)
  useEffect(() => {
    const currentSKU = form.getValues('sku');
    if (watchedName && watchedName.trim().length >= 3 && !currentSKU) {
      const timer = setTimeout(() => {
        handleGenerateSKU();
      }, 500);
      return () => clearTimeout(timer);
    }
  }, [watchedName, watchedCategoryId]);
  ...
  const onSubmit = async (data: ProductInput) => {
    ...
    const exists = await checkSKUExists(data.sku);
    if (exists) {
      setSkuError('Este SKU já existe. Use um SKU diferente.');
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    ...
                    <FormControl>
                      <Input placeholder="Ex: Mouse Wireless Pro" {...field} />
                    </FormControl>
                    <FormMessage />
                  </FormItem>
                )}
