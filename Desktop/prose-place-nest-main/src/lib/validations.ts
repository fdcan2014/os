...
export const PurchaseOrderItemSchema = z.object({
  ...
  quantity: z.number().min(1, 'Quantidade deve ser no mínimo 1').max(999999, 'Quantidade muito alta'),
  unit_cost: z.number().min(0, 'Custo unitário não pode ser negativo').max(999999999, 'Custo muito alto'),
...
export const PurchaseOrderSchema = z.object({
  ...
  location_id: z.string().uuid('Local é obrigatório'),
  order_date: z.string().min(1, 'Data do pedido é obrigatória'),
  expected_date: z.string().nullable().optional().or(z.literal('')).transform(val => val === '' ? null : val),
  notes: z.string().max(2000, 'Observações devem ter menos de 2000 caracteres').nullable().optional().or(z.literal('')).transform(val => val === '' ? null : val),
...
export const SupplierInvoiceSchema = z.object({
  invoice_number: z.string().min(1, 'Número da fatura é obrigatório').max(50, 'Número deve ter menos de 50 caracteres'),
  supplier_id: z.string().uuid('Fornecedor é obrigatório'),
  purchase_order_id: z.string().uuid().nullable().optional(),
  invoice_date: z.string().min(1, 'Data da fatura é obrigatória'),
  due_date: z.string().nullable().optional().or(z.literal('')).transform(val => val === '' ? null : val),
  subtotal: z.number().min(0).default(0),
  tax_amount: z.number().min(0).default(0),
  total: z.number().min(0).default(0),
  notes: z.string().max(2000).nullable().optional().or(z.literal('')).transform(val => val === '' ? null : val),
});
...
export const SupplierPaymentSchema = z.object({
  invoice_id: z.string().uuid('Fatura é obrigatória'),
  amount: z.number().min(0.01, 'Valor deve ser maior que zero').max(999999999, 'Valor muito alto'),
  payment_date: z.string().min(1, 'Data do pagamento é obrigatória'),
  payment_method_id: z.string().uuid().nullable().optional(),
  reference: z.string().max(100).nullable().optional().or(z.literal('')).transform(val => val === '' ? null : val),
  notes: z.string().max(500).nullable().optional().or(z.literal('')).transform(val => val === '' ? null : val),
});
